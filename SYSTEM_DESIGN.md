...
- **`estimated_duration` is misleadingly named.** The `script-worker` writes a pre-TTS word-count estimate to a DynamoDB field called `estimated_duration` (`autopilot-lambda/script-worker/handler.py:163`, computed as `int(word_count / 120 * 60)`). The `voice-worker` then writes the actual rendered duration to a *separate* field, `audio_duration`. Anything that reads `estimated_duration` thinking it's the real value will truncate the assembly timeline — the same class of bug we just fixed in `voice-worker`. The current assembler does fall back to `estimated_duration` when `audio_duration` is missing (`autopilot-lambda/assembler/handler.py:_assemble`), which is a latent risk if `voice_done` is ever skipped. Suggested rename: `estimated_duration` → `script_estimated_duration`, with assembler updated to fail loudly rather than fall back. Not yet implemented; tracked as schema risk.
